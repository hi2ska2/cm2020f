...
for i= interface1+1 : interface2-1
    y_analytic(i,1)=1/2*(x(i)-0.8e-9)*(x(i)-0.8e-9-5e-9)/(eps0*eps_si)+1/2*-3*0.8e-9*q*Nacc*5e-9/(eps0*eps_si);
end
for i= interface2 : 1/2*(x(i)-1.6e-9-5e-9)*-3*0.8e-9*q*Nacc*5e-9/(eps0*eps_si)/0.8e-9;
end
plot(x,y_analytic);
xlabel('x[nm]')
ylabel('V')
