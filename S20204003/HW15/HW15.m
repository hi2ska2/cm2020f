...
% Answer 1 (non-linear Poisson)
elec_Poisson=elec/1e6;
...
for Newton=1:50
    ...
    for ii=2:N-1
        ...
        res(2*N-1,1) = phi(N,1) - k_B*T/q*log(Ndop1/nint);
        Jaco(2*N-1,2*N-1) = 1;
        n_av1 = 0.5*(elec(ii+1,1)+elec(ii,1));
        n_av2 = 0.5*(elec(ii,1)+elec(ii-1,1));
        dphidx1 = (phi(ii+1,1)-phi(ii,1))/dx;
    ...
    maximum_update_elec(Newton,1)=max(abs(save_update(2:2:2*N,Newton)))/1e6;
    
end

% Answer 2 (Self Consistent Poisson-Continuity)
elec_DD=elec/1e6;
