...
% electron_Sch2 = zeros(N,length(Vg)); % Electron density, /m^3
...
for i = 1:length(Vg)
    ...
    for iNewton = 1:Schrhodinger_loop
        ...
        for poisson_newton = 1:1
            ...
            update = Jaco \ (-res);
            phi(:,i) = phi(:,i) + update;
            %             electron_Sch(interface1+1:interface2-1,i) = electron_Sch(interface1+1:interface2-1,i).*exp(update/thermal);
%             electron_Sch2(interface1+1:interface2-1,i) = ni*exp(phi(interface1+1:interface2-1,i)/thermal);
...
% electron_Sch2 = electron_Sch2/1e6; %/cm^3
